...
def play_game(user):
    while True:
        ...
        command = int(input("Choose an option:"
                            "\n1: move\n2: pick up"
                            "\n3: inventory\n4: display map of descovered rooms"
                            "\n5: quit\n\nOption: "))

        if command == Option.MOVE:
            direction = input("Provide direction (left|right|up|down): ")
            user.move(direction)
        elif command == Option.PICK_UP:
        ...
        elif command == Option.INVENTORY:
            user.show_inventory()
        elif command == Option.PRINT_MAP:
            user.display_map()
        elif command == Option.QUIT:
